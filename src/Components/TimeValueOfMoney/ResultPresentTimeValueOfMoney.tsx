...
const ResultPresentTimeValueOfMoney = ({ presentValue, nominalValue, accruedInterest }: CompoundInterestCalculateResult) => {
  ...
  return (
    <div className='result'>
      <h2 tabIndex={checkTabIndex} className='result__item'>Wartość Obecna Inwestycji: {jsxPresentValue}</h2>
      <h2 tabIndex={checkTabIndex} className='result__item'>Wartość Nominalna wyniesie: {jsxNominalValue}</h2>
      <h2 tabIndex={checkTabIndex} className='result__item'>Różnica: {jsxAccruedInterest}</h2>
    </div>
  )
}
