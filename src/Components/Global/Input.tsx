...
const Input = ({ inputType = 'number', content, inputRequired = true, inputState, swapOptionalState, handleSetInputState, handleSwapOptionalState }: InputProps) => {
  ...
  const minInNumberInput = inputType === 'number' ? "0" : null


  return (
    <label className='form__label'>
      <input min={minInNumberInput ?? undefined} className={inputClassName} type={inputType} required={inputRequired} onChange={handleInputChange} value={inputState} />
